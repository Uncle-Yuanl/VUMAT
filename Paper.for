...
*
      ...
      xnu32 = xnu23 * E3 / E2
*
	G1tc = props(i_pro_G1tc)
	G1cc = props(i_pro_G1cc)
	G2tc = props(i_pro_G2tc)
	G2cc = props(i_pro_G2cc)
*
...
*     *     stressNew, eigen )
	call CHANG3d  ( nblock, nDmg,
     *       f1t, f2t, f3t, f1c, f2c, f3c, f12, f23, f13, 
     *       e1t, e2t, e3t, e1c, e2c, e3c, e12t, e23t, e13t,
     *       G1tc, G1cc, G2tc, G2cc,
     *     stateNew(1,i_svd_DmgFiberT),
     *     stateNew(1,i_svd_DmgFiberC),
     *     stateNew(1,i_svd_DmgMatrixT),
     ...
     *     stateNew(1,i_svd_statusMp), 
     *     stressNew, eigen,
     *     stateNew(1,i_svd_strain), charLength)
...
***********************************************************************
*      CHANG3d  w/ Modified Puck: Evaluate CHANG-CHANG  3d failure  *
*      criterion for fiber , Puck for matrix                        *
***********************************************************************
	subroutine CHANG3d ( nblock, nDmg,
     *       f1t, f2t, f3t, f1c, f2c, f3c, f12, f23, f13, 
     *       e1t, e2t, e3t, e1c, e2c, e3c, e12t, e23t, e13t,
     *       G1tc, G1cc, G2tc, G2cc,
     *       dmgFiberT, dmgFiberC, dmgMatrixT, dmgMatrixC,
...
*
	...
	e13tInv = zero       
*     
	if ( e1t .gt. zero )  e1tInv = one/e1t 
	if ( e2t .gt. zero )  e2tInv = one/e2t
	if ( e3t .gt. zero )  e3tInv = one/e3t
	if ( e1c .gt. zero )  e1cInv = one/e1c
	if ( e2c .gt. zero )  e2cInv = one/e2c
	if ( e3c .gt. zero )  e3cInv = one/e3c
	if ( e12t .gt. zero )  e12tInv = one/e12t
	if ( e23t .gt. zero )  e23tInv = one/e23t
	if ( e13t .gt. zero )  e13tInv = one/e13t
        
      do k = 1, nblock
		if ( statusMp(k) .eq. one ) then
*	 statusMp = 1 开始失效   = 0  删除
*              
		s11 = stress(k, i_s33_Xx)
		s22 = stress(k, i_s33_Yy)
		s33 = stress(k, i_s33_Zz)
...
*           
*		0.18 是单层的厚度  此处为单一厚度                
		charLength(k) = SQRT((charLength(k)**3)/0.)
		e11ft = 2.0* G1tc/f1t/charLength(k)
		e11fc = 2.0* G1cc/f1c/charLength(k)
		e22ft = 2.0* G2tc/f2t/charLength(k)
		e22fc = 2.0* G2cc/f2c/charLength(k)
                
*         Evaluate Fiber modes
		if ( s11 .gt. zero ) then
*          --Tensile Fiber Mode                    
			rft = ( e11*e1tInv )**2 + ( e12*e12tInv )**2 
     *			+ ( e13*e13tInv )**2
			if ( rft .ge. one ) then
				fiberdmg = 1
				dmgFiberT(k)=abs(e11ft*(1.0-e1t/e11)/(e11ft-e1t))
...
*          --Compressive Fiber Mode                  
			...
			if ( rfc .ge. one ) then
				fiberdmg = 1
				dmgFiberC(k)=abs( e11fc*(1.0+e1c/e11)/(e11fc-e1c))
...
*        Evaluate Matrix Mode
		if ( (s22 + s33 ) .gt. zero ) then
*        --Tensile Matrix Mode
			rmt = (( e22 + e33 )*e2tInv )**2
     *			+( e23**2 - e22*e33 )*e23tInv**2 + ( e13*e13tInv )**2 
     *			+ ( e12*e12tInv )**2
			if ( rmt .ge. one ) then
				matrixdmg = 1
				dmgMatrixT(k) =abs(e22ft*(1.0-e2t/e22)/(e22ft-e2t))
...
*        --Compressive Matrix Mode
			rmc = (( half*e2c*e12tInv)**2 - 1)*((e22 + e33)*e2cInv) 
     *			+ (( e22 + e33 )*e2tInv )**2 
     *			+ ( e23**2 - e22*e33 )*e23tInv**2 
     *			+ ( e13*e13tInv)**2 + ( e12*e12tInv )**2
			if ( rmc .ge. one ) then
				matrixdmg = 1
				dmgMatrixC(k) = abs(e22fc*(1.0+e2c/e22)/(e22fc-e2c))
...
*		  
		if ( enomMax .gt. eMax .or.
     *		 enomMin .lt. eMin .or.
     *		 dmgFiberT(k) .ge. one .or.
     *		 dmgFiberC(k) .ge. one) then
			statusMp(k) = zero
		end if
*
		nDmg = fiberdmg + matrixdmg
*		
          end if
*     
	end do
